...
export default async function OpenGraphImage() {
  ...
  const c = THEMES[b.theme].colors;
  const font = await fs.readFile(path.join(process.cwd(), "assets", "fonts", "NotoSansKR-Bold.otf"));
  return new ImageResponse(
    (
      <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", justifyContent: "space-between", padding: 72, background: `linear-gradient(135deg, ${c.brandDeep} 0%, ${c.brand} 70%, ${c.brand} 100%)`, color: "white", fontFamily: "Noto Sans KR" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 16, fontSize: 28, letterSpacing: 6, color: c.accent }}>농축협 디지털 프로젝트과정 · 공식 제작 도구</div>
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <div style={{ display: "flex", fontSize: 96, lineHeight: 1.1 }}>소재 하나로,</div>
          <div style={{ display: "flex", fontSize: 96, lineHeight: 1.1 }}>
            산출물&nbsp;<span style={{ color: c.accent }}>다섯.</span>
          </div>
