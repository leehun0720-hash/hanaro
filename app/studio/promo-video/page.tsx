...
export default async function PromoVideoPage({ searchParams }: PageProps<"/studio/promo-video">) {
  ...
  const [photoUrls, googleKey] = await Promise.all([projectPhotoUrls(projects), getSecret("GOOGLE_API_KEY")]);
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      <PromoClient initial={initial} projects={projects} photoUrls={photoUrls} preselect={preselect} credits={costFor("promo_video", costs)} googleReady={Boolean(googleKey)} />
    </div>
  );
}
